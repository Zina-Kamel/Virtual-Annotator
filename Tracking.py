...
class handDetector():
    ...
    def fingersUp(self):
        ...
        if (self.landmarks_list[self.finger_tip_id[0]][1] < self.landmarks_list[self.finger_tip_id[0] - 1][1]):
            fingers.append(1)
        else:
            fingers.append(0)
        
        for i in range(1,5):
            if (self.landmarks_list[self.finger_tip_id[i]][2] < self.landmarks_list[self.finger_tip_id[i] - 2][2]):
                fingers.append(1)
            else:
                fingers.append(0)
